...
function remplir_table(tab) {
	//let tab = valeurs_tableau(); // erreur console "Identifier 'tab' has already been declared (at cactpot.js:91:6)"
	//Recuperer le tableau array en table
	let liste_tds = document.getElementsByTagName("td"); // pas id mais tagnam pluseirus elements

    for (let i= 0; i < liste_tds.length; i++) {   // DTF un s dans td
    //Je récupère le contenu de td
    (liste_tds[i].innerHTML) = tab[i]; // Ne pas oublier le but de lexo recuperer le tab qui est l'argument
	}
return;
}
...
function somme_cases(id_cases) {
	...
	let td = document.getElementById(id_cases[i]); //let td =parseInt(document.getElementById(id_cases[i]).innerHTML 
	let contenu_td =td.innerHTML;
	let entier_td = parseInt(contenu_td);
	somme += entier_td; // somme += td;
	}
	return somme;
}
